...
// Elements for Song Detail Page (will not be used directly when clicking a song, but still loaded)
...
// Function to show the song detail page (kept, but not called from song list click)
...
function renderSongList() {
  ...
  if (songs.length === 0) {
    songListElement.innerHTML =
      '<li class="loading-songs">No songs available.</li>';
    return;
  }
  songs.forEach((song, index) => {
    ...
    // When a song item is clicked, immediately load & play the song then show the player page
    listItem.addEventListener("click", () => {
      currentSongIndex = index;
      loadSong(songs[currentSongIndex]);
      playTrack();
      showPlayerPage(); // Go directly to the music player page
    });

    // Event listener for hover
    listItem.addEventListener("mouseenter", () => {
      // Only activate video background if we are on the home page
    ...
    listItem.addEventListener("mouseleave", () => {
      // Hide video background only if we are on the home page
      if (homePage.classList.contains("active")) {
        backgroundVideoContainer.classList.remove("active");
        backgroundVideo.pause(); // Pause video when mouse leaves
        backgroundVideo.src = ""; // Clear src so it doesn't play in the background
        backgroundVideo.load();
...
function loadSong(song) {
  if (!song) {
    ...
    lyricsContainer.innerHTML = "<p>Lyrics not available.</p>"; // Change textContent to innerHTML
    audioPlayer.src = "";
...
function playTrack() {
  ...
  isPlaying = true;
  audioPlayer.play().catch((error) => console.error("Error on play:", error));
  updatePlayPauseIcon();
}
...
audioPlayer.addEventListener("timeupdate", () => {
  if (audioPlayer.duration) {
    ...
    lyricLines.forEach((line, index) => {
      ...
      // Determine the end time of this lyric line. If this is the last line, assume it ends at the end of the song.
    ...
    // --- Auto-scroll lyrics only if the highlighted line is not visible ---
    if (highlightedLine) {
      const containerRect = lyricsContainer.getBoundingClientRect();
      const lineRect = highlightedLine.getBoundingClientRect();

      // Check if the line is outside the container viewport
...
function init() {
  ...
  if (songs.length > 0) {
    ...
    albumArtPlayer.src =
      "https://placehold.co/100x100/3a3a4e/e0e0e0?text=Music";
  ...
  updatePlayPauseIcon();
  updateRepeatButtonUI();
  showHomePage(); // Start from the song list page
